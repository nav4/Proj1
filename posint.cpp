...
int PosInt::compare (const PosInt& x) const {
	if (digits.size() < x.digits.size())
		...
		return compareDigits(&digits[0], digits.size(), &x.digits[0], x.digits.size());
}
...
void PosInt::divremArray (int* q, int* r, const int* x, int xlen, const int* y, int ylen){
	// Copy x into r
	for (int i=0; i<xlen; ++i)
		r[i] = x[i];
...
void PosInt::divrem (PosInt& q, PosInt& r, const PosInt& x, const PosInt& y) {
	if (y.digits.empty())
		throw MPError("Divide by zero");
	else if (&q == &r)
		throw MPError("Quotient and remainder can't be the same");
